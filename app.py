# -*- coding: utf-8 -*-
"""
RAG 知识库问答 Web 应用（优化版）
功能：
- 上传 PDF，自动构建向量知识库
- 基于 DeepSeek 大模型进行智能问答
- 优化检索策略，提高回答准确率
"""
...
with st.sidebar:
            ...
            with st.spinner("⏳ 正在解析 PDF 并构建向量知识库..."):
                try:
                    ...
                    text_splitter = RecursiveCharacterTextSplitter(
                        chunk_size=600,       # 稍大一些，保留更多上下文
                        chunk_overlap=150,    # 重叠更多，避免关键信息被切断
                        separators=["\n\n", "\n", "。", "！", "？", "；", " ", ""]  # 中英文分隔符
                    )
                    ...
                    embeddings = HuggingFaceEmbeddings(
                        ...
                        model_kwargs={'device': 'cpu'},  # 强制使用 CPU，避免 MPS 兼容问题
                    ...
                    st.session_state.messages = []  # 清空历史对话
    ...
    with st.expander("⚙️ 高级检索设置"):
        k_value = st.slider(
            ...
            max_value=10,
            value=6,
            help="从知识库中检索多少个最相关的文本块。数值越大，上下文越丰富，但可能引入噪音。"
        )
...
if prompt := st.chat_input("请输入你的问题（例如：这篇论文的主要贡献是什么？）"):
    if st.session_state.vector_store is None:
        ...
        # 检索相关文本块
        with st.spinner(f"🔍 正在从知识库中检索 {k_value} 个最相关段落..."):
